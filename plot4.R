...
### Plot4 reads household_power_consumtion.txt creates four plots based on a designated
### subset of the table.
...
for(counter in 1:nrow(data_project)){
  if(counter==1){
    ...
    #Date_Time <- as.data.frame(Date_Time, stringsAsFactors = FALSE)
  }
...
### Open png device and set resolution, create four plots and turn off png device.
png(file = "plot4.png", width = 480, height = 480)

par(mfcol = c(2,2),mar=c(2, 4.1, 4.1, 8.5), xpd=TRUE)
...
### Third Plot
with(data_project, plot(Voltage ~ Date_Time, type="l", ylab = "Global Active Power"))

### Fourth Plot
with(data_project, plot(Global_reactive_power ~ Date_Time, type="l", ylab = "Global Reactive Power", xlab = ""))
